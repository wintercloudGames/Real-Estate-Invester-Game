...
@export var max_zoom_distance: float = 20
@export var max_distance: float = 50.0

# --- Inertia / Throw Settings ---
@export var friction: float = 0.93
@export var momentum_multiplier: float = 1.5
...
func _process(delta):
	...
	# Only allow keyboard movement if not currently dragging
	if not _is_dragging:
...
func _input(event):
	...
	# 2. Handle Drag Initiation (Right Click)
	if event is InputEventMouseButton:
...
func handle_zoom(delta: float):
	var current_height = cam.position.y
	var new_height = lerp(current_height, _target_zoom_distance, zoom_speed * delta)

	cam.position.y = new_height
	cam.position.z = new_height * 0.6 
